...
# def Hough_transform(threshold_image, nol=6):
#     '''
#     :param threshold_image: ảnh nhị phân (Canny edge)
#     :param nol: số line muốn giữ lại (line dài nhất)
#     :return:
#         linesP: list các line dạng [x1, y1, x2, y2, length]
#     '''
#     h, w = threshold_image.shape[:2]

#     # Tìm line bằng Hough Transform
...
#         d = math.sqrt((l[0] - l[2]) ** 2 + (l[1] - l[3]) ** 2)
#         if d < 0.5 * max(h, w):
#             d = 0  # bỏ line quá ngắn
#         dist.append(d)

#     dist = np.array(dist).reshape(-1, 1, 1)
#     linesP = np.concatenate([linesP, dist], axis=2)

#     # Sắp xếp theo độ dài line và lấy nol line dài nhất
#     linesP = sorted(linesP, key=lambda x: x[0][-1], reverse=True)[:nol]
