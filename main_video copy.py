...
while cap.isOpened():
    ...
    results = model_LP(source_img, conf=0.25)
    ...
    for i, box in enumerate(results[0].boxes):
        ...
        for cnt in cont:
            ...
            ratiochar = cw / ch
            char_area = cw * ch
            if (0.005 * roiarea < char_area < 0.15 * roiarea) and (0.2 < ratiochar < 1.0):
        ...
        # ====== Cơ chế ổn định biển số ======
        final_plate = ""
        if plate_text:
            if plate_text not in plate_memory:
                plate_memory[plate_text] = 1
            else:
                plate_memory[plate_text] += 1

            if plate_memory[plate_text] >= STABLE_THRESHOLD:
                final_plate = plate_text

        # Vẽ kết quả lên frame
        cv2.rectangle(LP_detected_img, (x1, y1), (x2, y2), (0, 255, 0), 3)
        if final_plate:
            cv2.putText(LP_detected_img, final_plate, (x1, y1 - 10),
                        cv2.FONT_HERSHEY_DUPLEX, 2, (255, 255, 0), 2)
            print(f"✅ Biển số {i} (ổn định): {final_plate}")
        else:
